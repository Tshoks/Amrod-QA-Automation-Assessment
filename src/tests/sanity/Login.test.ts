...
test.beforeAll(async () => {
  [loginData, loginNegativeData] = await Promise.all([
    getLoginTestData(),
    getLoginNegativeTestData(),
  ]);
});
...
export class LoginTest extends BaseTest {
  public static loginWithValidCredentials(): void {
    test("Login with valid credentials @sanity", async ({ page }, testInfo) => {
      ...
      await this.step(
        page,
        testInfo,
        "Step 3: Submit login and validate successful login",
        async () => {
  ...
  public static loginWithInvalidCredentials(): void {
    test("Login with invalid credentials @sanity", async ({
      page,
    }, testInfo) => {
      const loginPage = new LoginPage(page);
      ...
      await this.step(
        page,
        testInfo,
        "Step 3: Submit login and validate invalid credentials error",
        async () => {
          await loginPage.clickLoginButton();
          await loginPage.validateInvalidCredentials(
            loginNegativeData.invalidCredentialsMessage,
          );
        },
      );
    });
  ...
  public static loginValidationForRequiredFields(): void {
    test("Login required field validation @sanity", async ({
      page,
    }, testInfo) => {
      const loginPage = new LoginPage(page);
      ...
      await this.step(
        page,
        testInfo,
        "Step 2: Submit empty login form",
        async () => {
          await loginPage.clickLoginButton();
        },
      );
      ...
      await this.step(
        page,
        testInfo,
        "Step 3: Validate required field errors",
        async () => {
          await loginPage.validateRequiredFieldErrors(
            loginNegativeData.requiredFieldMessage,
          );
        },
      );
    });
